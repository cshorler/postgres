...
[
 ...
 AC_CACHE_CHECK([whether avx is supported], [ax_cv_have_avx_ext],
  ...
  if test "$ax_cv_have_mmx_ext" = yes; then
    ...
    AX_APPEND_COMPILE_FLAGS([-mmmx], [SIMD_FLAGS])
  fi

  if test "$ax_cv_have_sse_ext" = yes; then
    AC_DEFINE(HAVE_SSE,, [Support SSE (Streaming SIMD Extensions) instructions])
    AX_APPEND_COMPILE_FLAGS([-msse], [SIMD_FLAGS])
  fi

  if test "$ax_cv_have_sse2_ext" = yes; then
    AC_DEFINE(HAVE_SSE2,, [Support SSE2 (Streaming SIMD Extensions 2) instructions])
    AX_APPEND_COMPILE_FLAGS([-msse2], [SIMD_FLAGS])
  fi

  if test "$ax_cv_have_sse3_ext" = yes; then
    AC_DEFINE(HAVE_SSE3,, [Support SSE3 (Streaming SIMD Extensions 3) instructions])
    AX_APPEND_COMPILE_FLAGS([-msse3], [SIMD_FLAGS])
  fi

  if test "$ax_cv_have_ssse3_ext" = yes; then
    AC_DEFINE(HAVE_SSSE3,, [Support SSSE3 (Supplemental Streaming SIMD Extensions 3) instructions])
    AX_APPEND_COMPILE_FLAGS([-mssse3], [SIMD_FLAGS])
  fi

  if test "$ax_cv_have_sse41_ext" = yes; then
    AC_DEFINE(HAVE_SSE41,, [Support SSE4.1 (Streaming SIMD Extensions 4.1) instructions])
    AX_APPEND_COMPILE_FLAGS([-msse4.1], [SIMD_FLAGS])
  fi

  if test "$ax_cv_have_sse42_ext" = yes; then
    AC_DEFINE(HAVE_SSE42, 1,[Support SSE4.2 (Streaming SIMD Extensions 4.2) instructions])
    AX_APPEND_COMPILE_FLAGS([-msse4.2], [SIMD_FLAGS])
  fi

  if test "$ax_cv_have_avx_ext" = yes; then
    AC_DEFINE(HAVE_AVX,,[Support AVX (Advanced Vector Extensions) instructions])
    AX_APPEND_COMPILE_FLAGS([-mavx], [SIMD_FLAGS])
  fi
